...
/** Create a fresh zero-valued metrics object. */
...
/**
 * Symbol key used to store the per-instance metrics object on the Fastify app.
 * Using a symbol avoids name collisions with other decorators.
 */
const METRICS_KEY = Symbol('agentMetrics');

// ---------- Helpers ----------

function errorReply(app: FastifyInstance, reply: FastifyReply, status: number, code: string, message: string): void {
  const m = (app as unknown as Record<symbol, Metrics>)[METRICS_KEY];
  if (m) m.errorsTotal++;
...
export function registerRoutes(app: FastifyInstance, openaiClient: OpenAIClient): void {
  // Create instance-scoped metrics — no module-level mutable state
  ...
  app.post('/conversations', async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      const conversation = await openaiClient.createConversation(metadata);
      ...
      errorReply(app, reply, 500, 'internal_error', 'Failed to create conversation');
    }
  });
  ...
  app.get('/conversations', async (req: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      const list = await openaiClient.listConversations(offset, limit);
      await reply.send(list);
    } catch (err: unknown) {
      req.log.error({ err }, 'Failed to list conversations');
      errorReply(app, reply, 500, 'internal_error', 'Failed to list conversations');
    }
  });
  ...
  app.get('/conversations/:conversationId', async (req: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      if (!isValidId(conversationId)) {
        errorReply(app, reply, 400, 'bad_request', 'Invalid conversation ID format');
        return;
      }

      const detail = await openaiClient.getConversation(conversationId);
      if (!detail) {
        errorReply(app, reply, 404, 'not_found', `Conversation ${conversationId} not found`);
        return;
      }
      ...
      errorReply(app, reply, 500, 'internal_error', 'Failed to get conversation');
    }
  });
  ...
  app.post('/conversations/:conversationId/messages', async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    if (typeof content !== 'string' || content.length === 0) {
      errorReply(app, reply, 400, 'bad_request', 'Missing required field: content');
      return;
    }
    ...
    if (wantsStream) {
      ...
      try {
        ...
        const stream = openaiClient.sendMessageStream(conversationId, content, (chunk) => {
      ...
      try {
        ...
        const message = await openaiClient.sendMessage(conversationId, content);
        if (!message) {
          errorReply(app, reply, 404, 'not_found', `Conversation ${conversationId} not found`);
          return;
        }
        ...
        if (message.usage) {
          metrics.promptTokensTotal += message.usage.promptTokens ?? 0;
          metrics.completionTokensTotal += message.usage.completionTokens ?? 0;
        }
        ...
        errorReply(app, reply, 500, 'internal_error', 'Failed to send message');
      }
    }
  });
...
/** Retrieve the instance-scoped metrics from a Fastify app. */
export function getMetrics(app: FastifyInstance): Readonly<Metrics> {
  const m = (app as unknown as Record<symbol, Metrics>)[METRICS_KEY];
  if (!m) {
    throw new Error('Metrics not initialised — registerRoutes() must be called first');
  }
  return { ...m };
}
