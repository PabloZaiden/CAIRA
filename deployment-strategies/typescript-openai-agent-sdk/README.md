# CAIRA Deployment Strategy: TypeScript + Openai Agent Sdk

A complete, self-contained deployment strategy using Azure Container Apps with:

- **Frontend:** React + TypeScript (Vite build, Fastify BFF)
- **API:** TypeScript Fastify business API
- **Agent:** Agent container using OpenAI Agent SDK (Responses API)
...
```bash
npm run deploy:strategy -- deployment-strategies/typescript-openai-agent-sdk
```
...
```bash
npm run deploy:strategy:destroy -- deployment-strategies/typescript-openai-agent-sdk
```
...
| Agent               | 3000 | `/health`    | Agent container using OpenAI Agent SDK (Responses API)      |
...
├── infra/              # Terraform for ACR + Azure Container Apps deployment
