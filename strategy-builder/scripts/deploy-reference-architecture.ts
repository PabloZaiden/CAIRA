...
export interface TerraformOutputs {
  ...
  auth_tenant_id: string | undefined;
  api_token_scope: string | undefined;
  agent_token_scope: string | undefined;
  api_inbound_allowed_audiences: string[];
  agent_inbound_allowed_audiences: string[];
  api_inbound_allowed_caller_app_ids: string[];
  agent_inbound_allowed_caller_app_ids: string[];
}
...
function readOptionalStringArrayOutput(outputs: Record<string, { value?: unknown }>, key: string): string[] {
  const value = outputs[key]?.value;
...
async function getExistingOutputs(): Promise<TerraformOutputs | null> {
  ...
  try {
    ...
    return {
      ...
      auth_tenant_id: readOptionalStringOutput(outputs, 'auth_tenant_id'),
      api_token_scope: readOptionalStringOutput(outputs, 'api_token_scope'),
      agent_token_scope: readOptionalStringOutput(outputs, 'agent_token_scope'),
      api_inbound_allowed_audiences: readOptionalStringArrayOutput(outputs, 'api_inbound_allowed_audiences'),
      agent_inbound_allowed_audiences: readOptionalStringArrayOutput(outputs, 'agent_inbound_allowed_audiences'),
      api_inbound_allowed_caller_app_ids: readOptionalStringArrayOutput(outputs, 'api_inbound_allowed_caller_app_ids'),
      agent_inbound_allowed_caller_app_ids: readOptionalStringArrayOutput(
        outputs,
        'agent_inbound_allowed_caller_app_ids'
      )
    };
  } catch {
    return null;
...
async function writeEnvFile(strategyDir: string, outputs: TerraformOutputs): Promise<boolean> {
  ...
  const authLines = buildSharedAuthEnvLines(outputs);
...
function buildSharedAuthEnvLines(outputs: TerraformOutputs): string {
  const lines: string[] = [];

  if (outputs.api_token_scope) {
    lines.push(`API_TOKEN_SCOPE=${outputs.api_token_scope}`);
  }

  if (outputs.agent_token_scope) {
    lines.push(`AGENT_TOKEN_SCOPE=${outputs.agent_token_scope}`);
  }

  if (outputs.auth_tenant_id) {
    lines.push(`API_INBOUND_AUTH_TENANT_ID=${outputs.auth_tenant_id}`);
    lines.push(`AGENT_INBOUND_AUTH_TENANT_ID=${outputs.auth_tenant_id}`);
  }

  lines.push('API_INBOUND_AUTH_AUTHORITY_HOST=https://login.microsoftonline.com');
  lines.push('AGENT_INBOUND_AUTH_AUTHORITY_HOST=https://login.microsoftonline.com');

  if (outputs.api_inbound_allowed_audiences.length > 0) {
    lines.push(`API_INBOUND_AUTH_ALLOWED_AUDIENCES=${outputs.api_inbound_allowed_audiences.join(',')}`);
  }

  if (outputs.agent_inbound_allowed_audiences.length > 0) {
    lines.push(`AGENT_INBOUND_AUTH_ALLOWED_AUDIENCES=${outputs.agent_inbound_allowed_audiences.join(',')}`);
  }

  if (outputs.api_inbound_allowed_caller_app_ids.length > 0) {
    lines.push(`API_INBOUND_AUTH_ALLOWED_CALLER_APP_IDS=${outputs.api_inbound_allowed_caller_app_ids.join(',')}`);
  }

  if (outputs.agent_inbound_allowed_caller_app_ids.length > 0) {
    lines.push(`AGENT_INBOUND_AUTH_ALLOWED_CALLER_APP_IDS=${outputs.agent_inbound_allowed_caller_app_ids.join(',')}`);
  }
