 ...
 *
 * Uses a real Fastify instance with a mock OpenAIClient to test all
...
import { registerRoutes, getMetrics } from '../src/routes.ts';
import type { OpenAIClient } from '../src/openai-client.ts';
  ...
  const client: OpenAIClient = {
    ...
    checkHealth: async () => {
      const health: HealthResponse = {
        status: 'healthy',
        checks: [{ name: 'azure-openai', status: 'healthy', latencyMs: 5 }]
      };
      return health;
    },
...
describe('routes', () => {
  ...
  describe('POST /conversations/:conversationId/messages (JSON)', () => {
    ...
    it('tracks token usage in metrics', async () => {
      const createResp = await app.inject({
        method: 'POST',
        url: '/conversations'
      });
      ...
      const m = getMetrics(app);
      // Mock returns usage: { promptTokens: 10, completionTokens: 5 }
      expect(m.promptTokensTotal).toBe(10);
      expect(m.completionTokensTotal).toBe(5);
    });
  });
  ...
  describe('GET /health', () => {
    it('returns healthy status', async () => {
      ...
      expect(body.checks[0].name).toBe('azure-openai');
    });
