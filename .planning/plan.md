...
## Current resumed iteration after main devcontainer change

### Goal

Bring this worktree up to date with `main` without regressing the repo-wide CAIRA work, and specifically preserve the newly simplified single-devcontainer layout from `main`.

### Coding tasks for this iteration

1. Persist the new unattended merge request in `.planning/status.md`, confirm planning context, and record that `AGENTS.md` is still absent in this worktree.
2. Fetch and inspect `main` for the devcontainer simplification, identify the exact merge surface, and confirm which files moved or were deleted so the branch does not reintroduce dual devcontainers.
3. Merge `origin/main` into the current worktree, resolve conflicts in favor of the new single-devcontainer model where needed, and keep the CAIRA auth/docs/domain changes intact.
