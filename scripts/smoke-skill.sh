...
if [[ -n "${CAIRA_SMOKE_WORKDIR:-}" ]]; then
  WORK_DIR="${CAIRA_SMOKE_WORKDIR}"
  mkdir -p "${WORK_DIR}"
  if [[ -n "$(find "${WORK_DIR}" -mindepth 1 -maxdepth 1 -print -quit)" ]]; then
    echo "CAIRA_SMOKE_WORKDIR must point to an empty directory: ${WORK_DIR}" >&2
    exit 1
  fi
  KEEP_WORK_DIR="true"
else
  WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/caira-skill-smoke.XXXXXX")"
  KEEP_WORK_DIR="${CAIRA_KEEP_SMOKE_WORKDIR:-false}"
fi
...
cleanup() {
  if [[ "${KEEP_WORK_DIR}" != "true" ]]; then
    ...
    echo "Keeping smoke test workspace: ${WORK_DIR}"
  fi
}
trap cleanup EXIT
...
ensure_copilot() {
  ...
  local tool_dir="${CAIRA_SMOKE_TOOL_DIR:-${WORK_DIR}/.tools/npm-global}"
...
This is an unattended smoke test in a brand-new empty directory. Treat the following as the clarifications and approvals you need so you can complete the task in one shot:
...
- Preserve component-local validation style where practical, but keep the generated project small enough for a smoke test.
- In your final response, list the CAIRA reference paths you inspected or adapted and what you intentionally left out.
PROMPT

GENERATOR_OUTPUT="${WORK_DIR}/.caira-smoke-generator.out"
...
Verify the CAIRA skill smoke test result in this workspace. Do not modify files.
...
Inspect the workspace and the generator output in `.caira-smoke-generator.out`. The test passes only if all of these are true:
...
CAIRA_SMOKE_TEST_RESULT=PASS
or
CAIRA_SMOKE_TEST_RESULT=FAIL
PROMPT

VERIFIER_OUTPUT="${WORK_DIR}/.caira-smoke-verifier.out"
echo "Running Copilot verifier"
copilot "${COPILOT_ARGS[@]}" --prompt "${VERIFY_PROMPT}" | tee "${VERIFIER_OUTPUT}"

if grep -Eq '^[[:space:]]*CAIRA_SMOKE_TEST_RESULT=PASS[[:space:]]*$' "${VERIFIER_OUTPUT}" && ! grep -Eq '^[[:space:]]*CAIRA_SMOKE_TEST_RESULT=FAIL[[:space:]]*$' "${VERIFIER_OUTPUT}"; then
  echo "CAIRA skill smoke test passed"
else
  echo "CAIRA skill smoke test failed" >&2
  echo "Generator output: ${GENERATOR_OUTPUT}" >&2
  echo "Verifier output: ${VERIFIER_OUTPUT}" >&2
  if [[ "${KEEP_WORK_DIR}" != "true" ]]; then
    echo "Re-run with CAIRA_KEEP_SMOKE_WORKDIR=true to inspect the workspace." >&2
  fi
  exit 1
fi
