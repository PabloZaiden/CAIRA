...
| **Reference Architecture (RA)**  | A deployable CAIRA macro design under `strategy-builder/infra/reference-architectures/` (for example `strategy-builder/infra/reference-architectures/foundry_agentic_app/`) that combines Azure AI platform infrastructure with the baseline application-hosting infrastructure for supported Azure AI environments. |
